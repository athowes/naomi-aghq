...
```{r aghq, echo=FALSE, fig.align='center', fig.cap='Demonstration of Gauss-Hermite quadrature, adaption, and our principal components approach.'}
...
$$
where $m = \dim(\theta)$ and the Gauss-Hermite quadrature rule $z \in \mathcal{Q}(m, k)$ with weights $\omega:\mathcal{Q} \mapsto \mathbb{R}$ and $k$ points per dimension is adapted based upon
...
* Naomi can be used to assess probabilities targets have been met e.g. 90% of those who know their HIV status are on ART ("second 90"). Both TMB and PCA-AGHQ have biased inferences
