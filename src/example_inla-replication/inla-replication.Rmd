...
<!-- \begin{equation*} -->
<!-- - \log p(\btheta \, | \, \y) \sim f(x, \btheta) + \log p(x \, | \, \btheta, \y) -->
<!-- \end{equation*} -->
...
<!-- *Note: Does `TMB` keep the $-\frac{n}{2} \log(2\pi)$ constant here $-\log L^\star_f(\btheta) = -\frac{n}{2} \log(2\pi) + \frac{1}{2} \log \det (Q^\star(\btheta)) + f(\bm{\mu}^\star(\btheta), \btheta)$?* -->
...
2. Calculate negative Hessian at the mode.
The solution is likely to use some version of `h$env$spHess` although it's not clear which one.
3. Test points against the $\delta_z$ condition
...
Obtain marginals of $\tilde p_G(x \, | \, \theta, y)$ from above.
...
Probably going to need $Q^\star(\btheta)$ ($n \times n$ precision matrix) and $\bm{\mu}^\star(\btheta)$ ($n \times 1$ mean vector).
...
Precision matrix $Q^\star(\btheta)$ is the Hessian of the negative logarithm of the posterior evaluated at $\bm{\mu}^\star(\btheta)$, having $(i, j)$th entry
\begin{equation}
Q^\star(\btheta)_{i, j} = - \frac{\partial^{2} \log p(x \, | \, \btheta, \y)}{\partial x_{i} \partial x_{j}} \Big\rvert_{x = \bm{\mu}^\star(\btheta)}
\end{equation}
