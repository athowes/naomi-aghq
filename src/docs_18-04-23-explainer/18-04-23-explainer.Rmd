---
title: "Deterministic Bayesian inference methods for the Naomi model but make it verbose"
author: "Adam Howes"
date: "April 2023"
output:
  beamer_presentation:
...
subtitle: Preseneted to some future mathematical audience
...
```{r message=FALSE, echo=FALSE,  fig.cap="The Laplace approximation is good near the mode but not in the tails."}
laplace +
...
* Is it reasonable to just assume $p(\vartheta \, | \, y)$ is Gaussian?
...
* Now we can compute $p_{\texttt{LA}}(\theta, y)$ -- but what we really want is still $p(y)$ for which we need to integrate out $\theta$
* Hopefully^[Really: hopefully] $m$ is small enough that we can now tackle this with quadrature
* Given nodes $\mathcal{Q}$ and a weighting function $\omega$ we can compute
$$
...
* I will explain each of these^[The notation may not have been introduced properly, but hopefully the gist will still make sense], and whether or not they make Naomi an ELGM, in more detail in slides to follow
...
* The Naomi model uses both $\text{logit}$ and $\log$ (inverse) link functions
