...
abstract: |
  ...
  **Task** We implement the epilepsy example in Stan, `R-INLA`, `TMB`, `glmmTMB`, `tmbstan`, `aghq`, and a custom Laplace marginal version of `aghq`. We then compare the results of the inference methods.
---
...
fit11_samples <- aghq::sample_marginal(fit8, M = 1000)$samps %>%
  t() %>%
  as.data.frame() %>%
...
inla5 <- as.vector(t(fit2$summary.fixed[1:6, 1:2]))
inla6 <- as.vector(t(fit3$summary.fixed[1:6, 1:2]))
inla7 <- as.vector(t(fit4$summary.fixed[1:6, 1:2]))
tmb8 <- as.vector(t(data.frame(sd_out$par.random[1:6], sqrt(sd_out$diag.cov.random[1:6]))))
glmmtmb9 <- as.vector(t(summary(fit6)$coefficients$cond[, c("Estimate", "Std. Error")]))
tmbstan10 <- as.vector(t(summary(fit7)$summary[1:6, c(1, 3)]))

fit11_beta_samples <- fit8_samples[, 1:6]
aghq11 <- as.vector(t(data.frame(mean = apply(fit11_beta_samples, 2, mean), sd = apply(fit8_beta_samples, 2, sd))))
...
colnames(df) <- c("Stan", "EB-G", "EB-SL", "EB-L", "Grid-G", "Grid-SL", "Grid-L", "TMB", "glmmTMB", "tmbstan", "aghq", "adam", "type")
