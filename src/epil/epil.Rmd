...
output:
  bookdown::html_document2:
    ...
    theme: lumen
abstract: |
  **Background** Rue, Martino and Chopin (2009) use the epilespy example from the OpenBUGs manual as demonstration.
...
Section 5.2. of Rue, Martino and Chopin (2009) demonstrates the application of \texttt{R-INLA} to a generalised linear mixed model.
...
Hamiltonian Monte Carlo via the No-U-Turn algorithm, with one Markov chain of `iter = 1000` samples, and `warmup = 100` burn-in:
...
We can use various settings for the strategy, which controls how the latent field posterior marginals are approximated:
...
We integrate `random` out with a Laplace approximation:
...
For completeness, we check here that the objective functions as implemented in Stan and `TMB` are identical.
...
This likely isn't the same model, in terms of priors and so on, but it may be close.
...
One (strange!) approach is to use `glmmTMB` to get the `TMB` template which can be passed the `aghq`.
