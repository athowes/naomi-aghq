...
We integrate `random` out, which here corresponds to `c("epsilon", "nu")`, with a Laplace approximation:
...
Using just a few of them, we can see that there is a difference, but that difference is a constant so doesn't matter:
...
Let's just starting by trying to get the Laplace marginals for $\nu$:

```{r}
compile("epil_nu_index.cpp")
dyn.load(dynlib("epil_nu_index"))
```
...
Create a function to get the Laplace marginal for the $i$th element of $\nu$:

```{r}
xi_laplace_marginal_nu <- function(i, opt_theta, finegrid) {
  dat$i <- i
...
Now we can compute the Laplace marginal for all `r dat$N * dat$J` indices of $\nu$:

```{r}
fine_grid <- seq(-5, 5, length.out = 1000)

laplace_nu <- lapply(1:(dat$N * dat$J), xi_laplace_marginal_nu, opt_theta = opt_theta, finegrid = fine_grid) %>%
  bind_rows(.id = "index") %>%
  mutate(index = as.numeric(index))
```

There are too many marginals to plot them all, so here's a random subset of size nine:

```{r}
nine_random_indices <- sample(1:(dat$N * dat$J), size = 9)

laplace_nu %>%
  filter(index %in% nine_random_indices) %>%
  ggplot(aes(x = theta, y = pdf)) +
  geom_line() +
  facet_wrap(~index) +
  theme_minimal() +
  labs(x = "nu", y = "Posterior")
```
...
A `toggle` value of `0` corresponds to `nu` and a toggle value of `1` corresponds to `epsilon`:

```{r}
index_controller <- data.frame(
  "toggle" = c(rep(0, dat$N * dat$J),  rep(1, dat$N)),
  "i" = c(1:(dat$N * dat$J), 1:dat$N)
)
```
...
Start with `i = 1` for the first marginal:
...
#' Prepare the initialisation parameters if it's nu that we're toggling
#' We fix the nu parameter to zero, as we're using nu_i and nu_minus_i
if(dat$toggle == 0) {
  param_fixed_theta[["nu_i"]] <- param_fixed_theta$nu[i]
  param_fixed_theta[["nu_minus_i"]] <- param_fixed_theta$nu[-i]
...
get_random <- function(dat) {
  if(dat$toggle == 0) return(c("nu_minus_i", "epsilon"))
  if(dat$toggle == 1) return(c("nu", "epsilon_minus_i"))
}
