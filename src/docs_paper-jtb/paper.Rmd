...
(ref:us-alpha-xs) The parameters `us_alpha_xs` had the greatest difference in KS test statistics between GEB and GPCA-AGHQ to NUTS. This figure shows the element ``r paste0(ui_lambda_x$par)``. For this element, the NUTS potential scale reduction factor was `r ui_lambda_x$rhat` and effective sample size was `r ui_lambda_x$ess`.
