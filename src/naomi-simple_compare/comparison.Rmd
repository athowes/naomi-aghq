...
```{r}
rhats <- bayesplot::rhat(tmbstan$mcmc)
ess_ratio <- bayesplot::neff_ratio(tmbstan$mcmc)
...
These are the outcome variables which we might be interested in to monitor:

```{r}
names(tmb$fit$sample)[!(names(tmb$fit$sample) %in% pars)]
#' To-do: elicit from Jeff in particular which of these we should be focusing on the posterior output of
#' Guess would be the indicators HIV prevalence (rho_t1_out), ART coverage (alpha_t1_out), HIV incidence (lambda_t1_out)
```
# KS plots 
