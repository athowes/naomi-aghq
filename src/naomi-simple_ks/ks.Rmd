...
```{r}
pars_eval <- pars %in% c("beta_rho", "beta_alpha", "beta_anc_rho", "u_rho_x")
...
#' Save off KS results as example
saveRDS(
  beta_alpha$df_compare,
  file = "beta_alpha.rds"
)
```
...
beta_anc_rho <- histogram_and_ecdf("beta_anc_rho", i = 1, return_df = TRUE)
beta_anc_rho$plot

#' Save off KS results as example
saveRDS(
  beta_anc_rho$df_compare,
  file = "beta_anc_rho.rds"
)
```
