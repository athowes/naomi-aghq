...
Type objective_function<Type>::operator() ()
{
  ...
  PARAMETER(beta_rho_i);
  PARAMETER_VECTOR(beta_rho_minus_i);

  vector<Type> beta_rho(2); // Eventually this "2" shouldn't be hard-coded in
  int k = 0;
  for (int j = 0; j < 2; j++) {
    if (j + 1 == i) { // +1 because C++ does zero-indexing
      beta_rho(j) = beta_rho_i;
    } else {
      beta_rho(j) = beta_rho_minus_i(k);
      k++;
    }
  }
