...
```{r, echo=FALSE, fig.cap="I attended one of the best engineering schools in Canada", out.width = '90%'}
...
* One proposed solution is to use principal components analysis (PCA) and keep only the first $s < 24$ dimensions
...
gg4 <- gg
mvQuad::rescale(gg4, m = mu, C = cov, dec.type = 1, levels = c(3, 1))
...
* We run HMC for as long as it takes to get a good answer (around 3 days)
* Compare output of `TMB` with `aghq`: which is more similar to HMC?
* There are many ways you might compare posterior distributions
* We'll show Kolmogorov-Smirnov test results here
...
* Developed an approximate Bayesian inference method more accurate than `TMB` and faster than MCMC
* Scaled hyperparameter grid to >20 dimesions using PCA
* Relatively straightforward implementation using the `aghq` R package
