...
```{r, echo=FALSE, fig.cap="One of the best engineering schools in Canada", out.width = '90%'}
...
* Proposed solution use principal components analysis and keep only first $s < 24$ dimensions
* We will use $k = 3$ points per dimension

#

```{r, echo=FALSE, message=FALSE, fig.cap="Illustration of what this looks like for the 2D example, keeping only the first principal component, $s = 1$."}
gg3 <- gg
mvQuad::rescale(gg3, m = mu, C = cov, dec.type = 2)
...
```{r, echo=FALSE, fig.cap="This Scree plot suggests 10 or so dimensions is enough. We use $s = 8$.", out.width = '80%'}
...
* Use test data from Malawi
* Run HMC for as long as it takes to get a good answer
* Compare output of `TMB` with `aghq`: which is more similar to HMC?
* There are many ways you might do this: I'll show you Kolmogorov-Smirnov test results

#

Worse case TMB plot demonstrating KS test

#

Overall KS results plot
...
* Code and notebooks for this project are at `athowes.github.io/elgm-inf`
