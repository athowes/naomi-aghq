...
Type objective_function<Type>::operator() ()
{
  ...
  vector<Type> x(467); // Eventually this "467" shouldn't be hard-coded in
  int k = 0;
  for (int j = 0; j < 467; j++) {
  ...
  vector<Type> beta_rho = x.segment(x_starts(0), x_lengths(0));
  // PARAMETER_VECTOR(beta_rho);
  val -= dnorm(beta_rho, 0.0, 5.0, true).sum();

  vector<Type> beta_alpha = x.segment(x_starts(1), x_lengths(1));
  // PARAMETER_VECTOR(beta_alpha);
  val -= dnorm(beta_alpha, 0.0, 5.0, true).sum();

  vector<Type> beta_lambda = x.segment(x_starts(2), x_lengths(2));
  // PARAMETER_VECTOR(beta_lambda);
  val -= dnorm(beta_lambda, 0.0, 5.0, true).sum();

  vector<Type> beta_anc_rho = x.segment(x_starts(3), x_lengths(3));
  // PARAMETER_VECTOR(beta_anc_rho);
  val -= dnorm(beta_anc_rho, 0.0, 5.0, true).sum();

  vector<Type> beta_anc_alpha = x.segment(x_starts(4), x_lengths(4));
  ...
  vector<Type> u_rho_x = x.segment(x_starts(5), x_lengths(5));
  // PARAMETER_VECTOR(u_rho_x);
  vector<Type> us_rho_x = x.segment(x_starts(6), x_lengths(6));
  ...
  vector<Type> u_rho_xs = x.segment(x_starts(7), x_lengths(7));
  // PARAMETER_VECTOR(u_rho_xs);
  vector<Type> us_rho_xs = x.segment(x_starts(8), x_lengths(8));
  ...
  vector<Type> u_rho_a = x.segment(x_starts(9), x_lengths(9));
  // PARAMETER_VECTOR(u_rho_a);
  if(u_rho_a.size() > 0)
    val += SCALE(AR1(phi_rho_a), sigma_rho_a)(u_rho_a);

  vector<Type> u_rho_as = x.segment(x_starts(10), x_lengths(10));
  // PARAMETER_VECTOR(u_rho_as);
  if(u_rho_a.size() > 0)
    val += SCALE(AR1(phi_rho_as), sigma_rho_as)(u_rho_as);

  // Not sure what's going on with this parameter, I'm just going to leave it as it was before
  // vector<Type> u_rho_xa = x.segment(x_starts(?), x_lengths(?));
  ...
  vector<Type> u_alpha_x = x.segment(x_starts(11), x_lengths(11));
  // PARAMETER_VECTOR(u_alpha_x);
  vector<Type> us_alpha_x = x.segment(x_starts(12), x_lengths(12));
  ...
  vector<Type> u_alpha_xs = x.segment(x_starts(13), x_lengths(13));
  // PARAMETER_VECTOR(u_alpha_xs);
  vector<Type> us_alpha_xs = x.segment(x_starts(14), x_lengths(14));
  ...
  vector<Type> ui_lambda_x = x.segment(x_starts(18), x_lengths(18));
  ...
  vector<Type> ui_anc_rho_x = x.segment(x_starts(19), x_lengths(19));
  // PARAMETER_VECTOR(ui_anc_rho_x);
  val -= sum(dnorm(ui_anc_rho_x, 0.0, sigma_ancrho_x, true));

  vector<Type> ui_anc_alpha_x = x.segment(x_starts(20), x_lengths(20));
  ...
  vector<Type> log_or_gamma = x.segment(x_starts(21), x_lengths(21));
