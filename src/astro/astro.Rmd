...
We reanalyse the "estimating the mass of the Milky Way" example from @bilodeau2021stochastic.
...
kstable_astroebquad <- kstable(astroebquad) %>%
  pivot_longer(cols = everything()) %>%
  mutate(method = "EB", k = k_eb, s = NA, total_points = k_eb)

kstable_astropcaquad <- kstable(astroquad) %>%
  pivot_longer(cols = everything()) %>%
  mutate(method = "AGHQ-PCA", k = k_pca, s = s, total_points = k^s)

bind_rows(kstable_astroquad, kstable_astroebquad, kstable_astropcaquad) %>%
  gt::gt()
