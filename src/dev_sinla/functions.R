run_analysis_model1 <- function(sim_data, analysis_name) {
  ...
  print(ggplot(ecdf_tmbstan, aes(x = x, y = y)) +
...
run_analysis_model1_icar <- function(sim_data, analysis_name) {
  ...
  compile("model1_icar_index.cpp")
  dyn.load(dynlib("model1_icar_index"))
  ...
  pdf(paste0("model1-icar-plots-", analysis_name, ".pdf"), h = 8, w = 6.25)
  ...
  saveRDS(
    list("mcmc" = samples_tmbstan, "gaussian" = samples_gaussian, "laplace" = samples_laplace),
    paste0("model1-icar-samples-", analysis_name, ".rds")
  )
}
