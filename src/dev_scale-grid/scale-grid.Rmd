...
There are this many on the logit scale:

```{r}
data.frame(hypers) %>%
  filter(str_detect(hypers, "^logit_")) %>%
  nrow()
```

And this many on the log scale:
```{r}
data.frame(hypers) %>%
  filter(str_detect(hypers, "^log_")) %>%
  nrow()
```
...
For example if the hyper is on the logit scale then it looks as though it will have a higher SD than another hyper on the log scale based on the plot above.
To be more precise:

```{r}
data.frame(par = names(sd), sd = unname(sd)) %>%
  filter(str_detect(par, "^log_")) %>%
  summarise(mean_sd = mean(sd))
```

```{r}
data.frame(par = names(sd), sd = unname(sd)) %>%
  filter(str_detect(par, "^logit_")) %>%
  summarise(mean_sd = mean(sd))
```

Does this really reflect our intuititions about what is important to take greater account of?
...
Although it doesn't solve the above issue, another option to using the standard deviations is to use principle components analysis (PCA) to create a grid on a subspace of $\mathbb{R}^{24}$ which retains most of the variation.
...
obj <- function(theta) {
  mvtnorm::dmvnorm(theta, mean = mu, sigma = cov, log = TRUE)
}
...
pca_grid_1 <- pca_rescale(m = mu, C = cov, s = 1, k = 1)
pca_grid_3 <- pca_rescale(m = mu, C = cov, s = 1, k = 3)
pca_grid_5 <- pca_rescale(m = mu, C = cov, s = 1, k = 5)
pca_grid_7 <- pca_rescale(m = mu, C = cov, s = 1, k = 7)

plot +
  geom_point(
    data = data.frame(mvQuad::getNodes(pca_grid_7)) %>%
      mutate(weight = mvQuad::getWeights(pca_grid_7)),
...
opt_bfgs <- aghq::optimize_theta(ff, mu, control = default_control(method = "BFGS"))

# Perform regular AGHQ with a product grid
...
# Zero is the correct answer here, so hopefully these are close!
norm_bfgs_1$lognormconst
norm_bfgs_3$lognormconst
norm_bfgs_5$lognormconst
norm_bfgs_7$lognormconst

# What about with the new PCA grid?
...
# These don't look near zero! Something is going wrong
norm_pca_bfgs_1$lognormconst
norm_pca_bfgs_3$lognormconst
norm_pca_bfgs_5$lognormconst
norm_pca_bfgs_7$lognormconst
```

### Other 2D function

More complicated two dimensional function from Alex's [unit tests](https://github.com/awstringer1/aghq/blob/master/tests/testthat/setup-01-optimization.R) from the `aghq` package.
...
obj <- function(x) logfteta2d(x, c(y1, y2))
...
ground_truth <- data.frame(grid) %>%
  rowwise() %>%
  mutate(pdf = exp(objfunc2d(x = c(theta1, theta2))))
...
opt_bfgs <- aghq::optimize_theta(funlist2d, c(1.5, 1.5), control = default_control(method = "BFGS"))

grid_bfgs_1 <- mvQuad::createNIGrid(dim = 2, type = "GHe", level = 1)
# Error in rescale.NIGrid(grid_bfgs_3, m = opt_bfgs$mode, C = solve(opt_bfgs$hessian)) :
# (in rescale) no appropriate covariance matrix C
# mvQuad::rescale(grid_bfgs_1, m = opt_bfgs$mode, C = solve(opt_bfgs$hessian))

grid_bfgs_3 <- mvQuad::createNIGrid(dim = 2, type = "GHe", level = 3)
# Error in rescale.NIGrid(grid_bfgs_3, m = opt_bfgs$mode, C = solve(opt_bfgs$hessian)) :
# (in rescale) no appropriate covariance matrix C
# mvQuad::rescale(grid_bfgs_3, m = opt_bfgs$mode, C = solve(opt_bfgs$hessian))

grid_bfgs_5 <- mvQuad::createNIGrid(dim = 2, type = "GHe", level = 5)
# Error in rescale.NIGrid(grid_bfgs_3, m = opt_bfgs$mode, C = solve(opt_bfgs$hessian)) :
# (in rescale) no appropriate covariance matrix C
# mvQuad::rescale(grid_bfgs_5, m = opt_bfgs$mode, C = solve(opt_bfgs$hessian))

grid_bfgs_7 <- mvQuad::createNIGrid(dim = 2, type = "GHe", level = 7)
# Error in rescale.NIGrid(grid_bfgs_3, m = opt_bfgs$mode, C = solve(opt_bfgs$hessian)) :
# (in rescale) no appropriate covariance matrix C
# mvQuad::rescale(grid_bfgs_7, m = opt_bfgs$mode, C = solve(opt_bfgs$hessian))
...
pca_grid_bfgs_1 <- pca_rescale(m = opt_bfgs$mode, C = solve(opt_bfgs$hessian), s = 1, k = 1)
pca_grid_bfgs_3 <- pca_rescale(m = opt_bfgs$mode, C = solve(opt_bfgs$hessian), s = 1, k = 3)
pca_grid_bfgs_5 <- pca_rescale(m = opt_bfgs$mode, C = solve(opt_bfgs$hessian), s = 1, k = 5)
pca_grid_bfgs_7 <- pca_rescale(m = opt_bfgs$mode, C = solve(opt_bfgs$hessian), s = 1, k = 7)
...
#' AGHQ
{plot_points(grid_bfgs_1) + plot_points(grid_bfgs_3)} / {plot_points(grid_bfgs_5) + plot_points(grid_bfgs_7)}

#' PCA-AGHQ
{plot_points(pca_grid_bfgs_1) + plot_points(pca_grid_bfgs_3)} / {plot_points(pca_grid_bfgs_5) + plot_points(pca_grid_bfgs_7)}
...
results <- data.frame(
  ...
  "lognormconst" = c(
    truelognormconst2d,
    norm_bfgs_1$lognormconst, norm_bfgs_3$lognormconst, norm_bfgs_5$lognormconst, norm_bfgs_7$lognormconst,
    norm_pca_bfgs_1$lognormconst, norm_pca_bfgs_3$lognormconst, norm_pca_bfgs_5$lognormconst, norm_pca_bfgs_7$lognormconst
  )
)

ggplot(results, aes(x = method, y = lognormconst)) +
  geom_point(size = 1) +
