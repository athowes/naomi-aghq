...
```{r sd, fig.cap="Marginal standard deviations."}
...
```{r cov, fig.cap="Covariance matrix."}
C <- sd_out$cov.fixed
...
The average standard deviation for hyperparameters starting with `"log_"` is `r data.frame(par = names(sd), sd = unname(sd)) %>% filter(str_detect(par, "^log_")) %>% summarise(mean_sd = mean(sd)) %>% pull(mean_sd)` as compared with `r data.frame(par = names(sd), sd = unname(sd)) %>% filter(str_detect(par, "^logit_")) %>% summarise(mean_sd = mean(sd)) %>% pull(mean_sd)` for those starting with `"logit_"`.
...
The relative contributions of each principle component are:
...
```{r pc-loadings, fig.cap="PC loadings."}
...
```{r cov-reconstruction, fig.cap="PC loadings."}
E_s <- E[, 1:s]
...
For example, the version with 7 points looks as follows:

```{r pca-grid-7, fig.cap="AGHQ-PCA grid for Gaussian with $k = 7$."}
...
```{r fig.cap="Unit test for the Gaussian example"}
truelognormconst <- 0

results <- data.frame(
  "method" = c("Truth", paste0("aghq, k = ", c(1, 3, 5, 7)), paste0("aghq-pca, k = ", c(1, 3, 5, 7))),
  "lognormconst" = c(truelognormconst, sapply(norm_bfgs, function(x) x$lognormconst), sapply(norm_pca_bfgs, function(x) x$lognormconst)),
  "type" = as.factor(c("Truth", rep("AGHQ", 4), rep("AGHQ-PCA", 4)))
)
...
#' AGHQ
plot_points(grid_bfgs_7)

#' PCA-AGHQ
#' Something to fix here: points not being scaled to the right place
plot_points(pca_grid_bfgs_7)
