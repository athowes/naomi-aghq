...
pca_grid <- pca_rescale(m = mu, C = cov, s = 1, k = 5)
...
opt_bfgs <- aghq::optimize_theta(obj_list, mu, control = default_control(method = "BFGS"))
norm_bfgs_1 <- aghq::normalize_logpost(opt_bfgs, 1, 1)
exp(norm_bfgs_1$lognormconst)
```
...
grid_bfgs_2d_1 <- mvQuad::createNIGrid(dim = 2, type = "GHe", level = 1)
# Error in rescale.NIGrid(grid_bfgs_2d_3, m = opt_bfgs_2d$mode, C = solve(opt_bfgs_2d$hessian)) :
# (in rescale) no appropriate covariance matrix C
# mvQuad::rescale(grid_bfgs_2d_1, m = opt_bfgs_2d$mode, C = solve(opt_bfgs_2d$hessian))

grid_bfgs_2d_3 <- mvQuad::createNIGrid(dim = 2, type = "GHe", level = 3)
# Error in rescale.NIGrid(grid_bfgs_2d_3, m = opt_bfgs_2d$mode, C = solve(opt_bfgs_2d$hessian)) :
# (in rescale) no appropriate covariance matrix C
# mvQuad::rescale(grid_bfgs_2d_3, m = opt_bfgs_2d$mode, C = solve(opt_bfgs_2d$hessian))

grid_bfgs_2d_5 <- mvQuad::createNIGrid(dim = 2, type = "GHe", level = 5)
# Error in rescale.NIGrid(grid_bfgs_2d_3, m = opt_bfgs_2d$mode, C = solve(opt_bfgs_2d$hessian)) :
# (in rescale) no appropriate covariance matrix C
# mvQuad::rescale(grid_bfgs_2d_5, m = opt_bfgs_2d$mode, C = solve(opt_bfgs_2d$hessian))

grid_bfgs_2d_7 <- mvQuad::createNIGrid(dim = 2, type = "GHe", level = 7)
# Error in rescale.NIGrid(grid_bfgs_2d_3, m = opt_bfgs_2d$mode, C = solve(opt_bfgs_2d$hessian)) :
# (in rescale) no appropriate covariance matrix C
# mvQuad::rescale(grid_bfgs_2d_7, m = opt_bfgs_2d$mode, C = solve(opt_bfgs_2d$hessian))
...
#' AGHQ
{plot_points(grid_bfgs_2d_1) + plot_points(grid_bfgs_2d_3)} / {plot_points(grid_bfgs_2d_5) + plot_points(grid_bfgs_2d_7)}
