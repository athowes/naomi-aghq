...
(marginal_sd_plot <- data.frame(par = names(sd), sd = unname(sd)) %>%
  mutate(
...
ggsave("marginal-sd.png", plot = marginal_sd_plot, height = 3.5, w = 6.25)
```
...
Interesting to note that for the Cholesky adapted grid with `levels = c(1, 1, ..., 2, 1, ..., 1)` we see variation in all of the dimensions greater than or equal to the 16th.
...
```{r}
diff(as.matrix(quad$modesandhessians[, 1:24]))
```

Let's also verify that adaption is happening as we expect it to be:

```{r}
#' This is what the base grid looks like
mvQuad::getNodes(base_grid)
...
diff(mvQuad::getNodes(base_grid_copy))
```
...
(tv_plot <- ggplot(tv_df, aes(x = n, y = tv)) +
  geom_point() +
...
ggsave("tv-plot.png", plot = tv_plot, h = 3, w = 6.25)
```
...
(reduced_rank_plot <- {C_plot + theme(legend.position = "none") + labs(subtitle = "Full rank")} + {C_s_plot + labs(subtitle = "Reduced rank")})

ggsave("reduced-rank-plot.png", reduced_rank_plot, h = 3.5, w = 6.25)
```
