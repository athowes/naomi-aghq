...
Verifying that there are two grid points used in the 16th dimension:
...
Note that for the Cholesky adapted grid with `levels = c(1, 1, ..., 2, 1, ..., 1)` we see variation in all of the dimensions further than the one with a level above one.
I would assume this is related in some way to the Cholesky adaption being based on a lower triangular matrix.
This is something it would be worthwhile to work out in more detail.

```{r}
diff(as.matrix(quad$modesandhessians[, 1:24]))
```

Verifying that adaption is happening as we expect it to be:
