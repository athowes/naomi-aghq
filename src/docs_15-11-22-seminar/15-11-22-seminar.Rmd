...
* Consider small-areas $i = 1, \ldots, n$
* Simple random sample household-survey in each area, with sample sizes $m^\text{HS}_i$
...
* R package which implements the Laplace approximation for latent variable models using automatic differentiation (via `CppAD`)
  * For more about AD see e.g. @griewank2008evaluating. It's useful for getting the mode, Hessian
...
* Supposing you use $k$ quadrature points, then
