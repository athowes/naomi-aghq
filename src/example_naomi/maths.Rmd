...
* Likelihood for number of ANC clients specified only for year $Y[T_2]$ (**why?**) as
$$
...
* Let $\dot A_{\{x\}, \{s\}, \{a\}, t}$ be data for the number receiving ART **flag: confusion about the notation here. In general would be good to define precisely what is meant by $\{x\}$. Presumably the LHS below is a set, yet the RHS is a sum over the sets so I think just a number -- so how can the two be equal?**
$$
...
$$
where $\pi_{x', x, s, a, t} = \rho_{x', s, a, t} \cdot \alpha_{x', s, a, t} \cdot \gamma_{x', s, a, t}$ **flag: should this be $\gamma_{x', x, s, a, t}$ since it's people moving from $x'$ to $x$ (and so far no where in the RHS is there an $x$ which is suspicious)**
...
where
  ...
  * $\sigma_{\{x\},\{s\}\{a\}, t}^{\tilde{A}} = \sqrt{\sum_{s \in\{s\}} \sum_{a \in\{a\}} \sum_{x \in\{x\}} \sum_{x \sim x^', x=x \prime} N_{x^', s, a, t} \cdot \pi_{x^', x, s, a, t} \cdot \left(1-\pi_{x^', x, s, a, t}\right)}$
