...
3. Obtain a nested approximation to the posterior marginal of the $i$th latent effect by
$$
\tilde p(x_i \, | \, \y) = \sum_{\z \in \mathcal{Q}(m, k)} \tilde p_\text{AQ}(x_i \, | \, \btheta(\z), \y) \tilde p_\text{AQ} (\btheta(\z) \, | \, \y) \omega(\z).
$$
...
For each of the $k^m$ values of $\btheta(\z)$ we obtain $\tilde p_\text{AQ}(x_i \, | \, \btheta(\z), \y)$ by setting $\btheta = \btheta(\z)$ in the following two steps.

a) Use a Laplace approximation to obtain the unnormalised $\tilde p_\text{LA}(x_i, \btheta, \y)$
$$
...
b) Normalise $\tilde p_\text{LA}(x_i, \btheta, \y)$ using adaptive Gauss-Hermite quadrature to obtain
$$
\tilde p_\text{AQ}(x_i \, | \, \btheta, \y) = \frac{\tilde p_\text{LA}(x_i, \btheta, \y)}{\tilde p_{\text{AQ}}(\btheta, \y)},
$$
where the normalising constant is calculated using nodes from a Gauss-Hermite quadrature rule $v \in \mathcal{Q}(1, l)$ with $l$ nodes, and weights $\psi: v \in \mathcal{Q}(1, l) \to \mathbb{R}$ as follows
$$
\tilde p_{\text{AQ}}(\btheta, \y) = \sum_{v \in \mathcal{Q}(1, l)} \tilde p_\text{LA}(x_i(v), \btheta, \y) \psi(v),
$$
The nodes $v$ are adapted based on the mode and curvature at the mode of the Laplace approximation as follows
\begin{align*}
x_i(v) &= \hat{x_i} + \sigma_i v, \\
x_i(v) &= \argmax_{x_i} {\tilde p_\text{LA}(x_i, \btheta, \y)}, \\
\tau_i^2 &= \frac{\partial^2}{\partial^2 x_i} - \log \tilde p_\text{LA}(x_i, \btheta, \y) \rvert_{x_i = \hat x_i} \\
\sqrt{\tau_i^2} &= \sigma_i.
\end{align*}
As this quadrature is in one dimension, we can set $l = 5$ or more.
