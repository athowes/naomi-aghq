...
Concurrent modelling of data mitigates the limitations of any single data source, increases statistical power and prompts investigation into any conflicting information.
...
Situational requirements combined with the complexity of the model present a difficult Bayesian inference problem.
...
Below we provide an overview of the three components in the simplified model, highlighting aspects which make it a challenge for existing inferential approaches.
More complete mathematical details of this simplified model, as well as a \textsf{C++} template for the log-posterior, are provided in the appendix.

## Household survey component

Consider a country in sub-Saharan Africa where a household survey has taken place at $T_1$.
...
Let $W^\text{ANC}_i$ be the number of ANC clients, $X^\text{ANC}_i$ the number of those with ascertained status, $Y^\text{ANC}_i$ the number of those with positive status, and $Z^\text{ANC}_i$ the number of ANC clients already on ART prior to their first ANC visit.
ANC data from the year of the most recent survey is included in the model.

For women 15-49 the predicted number of ANC clients $\Psi_i$ is a log-linear model
$$
\log (\Psi_i) = \log (N_i) + \tilde \eta^\Psi
$$
...
We used three methods to assess the accuracy posterior distributions produced by each inferential method: (1) Kolmogorov-Smirnov (KS) tests performed on posterior marginals for the maximum difference between the two empirical cumulative distribution functions, (2) maximum mean discrepancy (MMD), and (3) Pareto-smoothed importance sampling (PSIS).
