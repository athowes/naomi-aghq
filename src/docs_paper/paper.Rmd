...
$m^{\rho^\text{ANC}}_I$, the number of ANC clients with ascertained status,
$y^{\rho^\text{ANC}}_I$, the number of those with positive status, and
$y^{\alpha^\text{ANC}}_I$, the number of those already on ART prior to their first ANC visit.
...
We then model aggregate ART attendance data $y^{\text{ART}}_I$ using a Gaussian approximation to a sum of binomials.
...
Let $\mathbf{y} = (y^{\theta}_I)$ for $\theta \in \{\hat \rho, \hat \alpha, \hat \kappa, \rho^\text{ANC}, \alpha^\text{ANC}, \text{ART}\}$ and $I \in \mathcal{I}$ be a concatenated vector of observations.
Here, we write the Naomi model fully in a small number of equations.
The ANC testing component and ART attendance component may be challenging.
...
Hyperparameter inferences are obtained according to some method which should be specified here.
...
grid <- expand.grid(
  theta1 = seq(-5, 5, length.out = 400),
  theta2 = seq(-5, 5, length.out = 400)
)
...
mu <- opt$par
cov <- as.matrix(sd_out$cov.fixed)
