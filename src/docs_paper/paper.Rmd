...
affiliation:
  ...
  - label: D
    ...
    authorsLabels: e4
abstract: |
  | Naomi is a spatial evidence synthesis model used to produce district-level HIV epidemic indicators in sub-Saharan Africa. Multiple outcomes of policy interest, including HIV prevalence, HIV incidence and antiretroviral therapy treatment coverage are jointly modelled using both household survey data and routinely reported health system data. Inference for Naomi is currently conducted using an empirical Bayes Gaussian approximation via Template Model Builder \texttt{TMB}. We propose a new inference method which combines the simplified integrated nested Laplace approximation approach of @wood2020simplified with adaptive Gauss-Hermite quadrature to enable fast and accurate inference for Naomi and other extended latent Gaussian models. Using data from Malawi, our method provides substantially more accurate inferences than default \texttt{TMB}, and is comparable to Hamiltonian Monte Carlo with the No-U-Turn sampler but substantially faster. By extending the \texttt{aghq} \textsc{R} package we facilitate flexible and easy use of our method when provided a \texttt{TMB} \textsf{C++} template for the model's log-posterior, enabling inference via integrated nested Laplace approximations for a larger class of models than was previously possible.
keyword-subclass: | 
...
Other data sources, such as routine health surveillance of antenatal care clinics, are more real-time but based on limited or non-representative samples of the population.
...
@eaton2021naomi specify a joint model linking small-area estimation models of HIV prevalence from household surveys, HIV prevalence among pregnant women from antenatal care clinics, and antiretroviral therapy (ART) coverage from routine health data collection.
...
We consider a simplified version of the model which is defined only at $T_1$ omitting nowcasting of $T_2$ and temporal projection to $T_3$.
...
Consider a country in sub-Saharan Africa where a household survey has taken place during at time $T_1$.
Let $x$ index district, $a$ five-year age band, and $s$ sex and.
...
\mathbf{H}(\btheta) &= \frac{\partial^2}{\partial x \partial x^\top} - \log p(\y, \x, \btheta) \rvert_{\x = \hat \x(\btheta)}.
\end{align*}
2. Generate a set of nodes $\z \in \mathcal{Q}(m, k)$ and weights $\omega: \z \in \mathcal{Q}(m, k) \to \mathbb{R}$ from a Gauss-Hermite quadrature rule with $m = \dim(\btheta)$, and $k$ nodes per dimension, which are then adapted based on the mode and lower Choleksy via $\btheta(\z) = \hat{\btheta} + \mathbf{L} \z$.
We typically set $k = 3$ such that there are $3^m$ nodes in total.
3. Use this quadrature rule to calculate the normalising constant $\tilde p_{\text{AQ}}(\y)$ as follows
$$
\tilde p_{\text{AQ}}(\y) = \sum_{\z \in \mathcal{Q}(m, k)} \tilde p_\text{LA}(\btheta(\z), \y) \omega(\z).
$$
4. For $i = 1, \ldots, n$ generate a set of nodes $\mathcal{X}_i$ via a Gauss-Hermite quadrature rule in one-dimension adapted based on the mode $\hat \x(\btheta)_i$ and standard deviation $\sqrt{\text{diag}[\mathbf{H}(\btheta)^{-1}]_i}$ to evaluate each latent field marginal Laplace approximation at.
...
\tilde p_\text{LA}(x_i, \btheta, \y) = \frac{p(x_i, \x_{-i}, \btheta, \y)}{\tilde p_\text{G}(\x_{-i} \, | \, x_i, \btheta, \y)} \Big\rvert_{\x_{-i} = \hat \x_{-i}(x_i, \btheta)}.
$$
7. Given $\{x_i, \tilde p_\text{AQ}(x_i \, | \, \y)\}_{x_i \in \mathcal{X}_i}$ create a spline interpolant to each posterior marginal on the log-scale, from which samples or relevant posterior summaries can be obtained.
...
* Our approach is similar to that of `inlabru` [@bachl2019inlabru] in the following ways, and different in the following other ways
