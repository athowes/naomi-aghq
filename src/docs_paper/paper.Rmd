...
Consider a latent Gaussian models (LGM) of the form
\begin{alignat}{2}
...
Here we describe the complete algorithm.
Given a C++ user template `model.cpp` for the unnormalised log posterior:
...
$$
where the normalising constant is calculated using points $\z \in \mathcal{Q}(m, k)$, where $m = \dim(\btheta)$ and $k$ is the number of points per dimension, and weights $\omega: \z \in \mathcal{Q}(m, k) \to \mathbb{R}$ as follows
$$
\tilde p_{\text{AQ}}(\y) = \sum_{\z \in \mathcal{Q}(m, k)} \tilde p_\text{LA}(\btheta(\z), \y) \omega(\z).
$$
3. For each of the $\dim(\mathcal{Q}(m, k))$ Laplace approximations in Step 2. store the mode and Hessian of the Gaussian approximation to the random effects used in the denominator
$$
\{ \bm{\mu}(\btheta(\z)), \bm{Q}(\btheta(\z)) \}_{\z \in \mathcal{Q}(m, k)}.
$$
4. Obtain a nested approximation to the posterior marginal of the $i$th latent effect by reusing the AGHQ nodes and weights from Step 2.
$$
\tilde p(x_i \, | \, \y) = \sum_{\z \in \mathcal{Q}(m, k)} \tilde p_\text{G}(x_i \, | \, \btheta(\z), \y) \tilde p_\text{AQ} (\btheta(\z) \, | \, \y) \omega(\z),
$$
where $p_\text{G}(x_i \, | \, \btheta(\z), \y)$ is a one-dimensional Gaussian with mean $\bm{\mu}(\btheta(\z))_i$ and variance $\bm{Q}(\btheta(\z))^{-1}_{ii}$.
