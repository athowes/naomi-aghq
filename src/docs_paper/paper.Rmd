---
title: Fast approximate Bayesian inference for small-area estimation of HIV using the Naomi model
author:
  - name: Adam 
    surname: Howes
...
Independent logistic regression models for HIV prevalence and ART coverage in the general population are specified on such that
\begin{align*}
\text{logit}(\rho_i) &= \eta^\rho_i, \\
\text{logit}(\alpha_i) &= \eta^\alpha_i,
\end{align*}
for certain choice of linear predictors $\eta^\rho_i$ and $\eta^\alpha_i$.
For the HIV incidence rate we model on the log scale $\log(\lambda_i) = \eta^\lambda_i(\{\rho_i, \alpha_i\}_{i \in \mathcal{I}})$, where the linear predictor depends on $\{\rho_i, \alpha_i\}_{i \in \mathcal{I}}$ for some $\mathcal{I}$.
