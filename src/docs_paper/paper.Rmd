...
affiliation:
  ...
  - label: D
    ...
    authorsLabels: e4
abstract: |
  | Naomi is a spatial evidence synthesis model used to produce district-level HIV epidemic indicators in sub-Saharan Africa. Multiple outcomes of policy interest, including HIV prevalence, HIV incidence and antiretroviral therapy treatment coverage are jointly modelled using both household survey data and routinely reported health system data. Inference for Naomi is currently conducted using an empirical Bayes Gaussian approximation via the \texttt{TMB} \textsc{R} package. We propose a new inference method which combines the simplified integrated nested Laplace approximation approach of @wood2020simplified with adaptive Gauss-Hermite quadrature to enable fast and accurate inference for Naomi and other extended latent Gaussian models. Using data from Malawi, our method provides substantially more accurate inferences than \texttt{TMB}, and is comparable to Hamiltonian Monte Carlo with the No-U-Turn sampler, but substantially faster. By extending the \texttt{aghq} \textsc{R} package we facilitate flexible and easy use of our method when provided a \texttt{TMB} \textsf{C++} template for the model's log-posterior, enabling inference via integrated nested Laplace approximations for a larger class of models than was previously possible.
keyword-subclass: | 
...
Other data sources, such as routine health surveillance of antenatal care (ANC) clinics, are more real-time but based on limited or non-representative samples of the population.
...
Below we provide an overview of the simplified model, highlighting the aspects which make it a challenge for existing inferential approaches.
...
Let $x$ index district, $a$ five-year age band, and $s$ sex.
...
Let $N_i \in \mathbb{N}$ be the population size, $\rho_i \in [0, 1]$ be HIV prevalence, $\alpha_i \in [0, 1]$ be antiretroviral therapy (ART) coverage, and $\lambda_i > 0$ be the annual HIV incidence rate.
Independent mixed effects models for HIV prevalence and ART coverage in the general population are specified on the logit scale such that
\begin{align*}
\text{logit}(\rho_i) &= \eta^\rho_i, \\
\text{logit}(\alpha_i) &= \eta^\alpha_i,
\end{align*}
for certain choice of linear predictors $\eta^\rho_i$ and $\eta^\alpha_i$.
For the HIV incidence rate we use a mixed effects model on the log scale $\log(\lambda_i) = \eta^\lambda_i(\{\rho_i, \alpha_i\}_{i \in \mathcal{I}})$, where the linear predictor depends on $\{\rho_i, \alpha_i\}_{i \in \mathcal{I}}$ for some $\mathcal{I}$.
...
The observed number of indicator cases is then $Y^{\hat \theta}_{\mathcal{I}} = M^{\hat \theta}_{\mathcal{I}} \cdot \hat \theta_{\mathcal{I}}$.
...
We assume that $\gamma_{x, x'} = 0$ unless $x = x'$ or the two districts are adjacent $x \sim x'$, and model $\gamma_{x, x'}$ using a multinomial logistic regression model where $\tilde \gamma_{x, x'}$ are the log-odds.
...
Instead, the structured additive predictor is redefined as $\bmeta = (\eta)_{i \in [N_n]}$, where $N_n \in \mathbb{N}$ depends on, but may not be equal to, $n$.
...
1. In the household survey component, the HIV incidence rate depends on the HIV prevalence and ART coverage linear predictors.
2. In the ANC testing component, the HIV prevalence and ART coverage depend upon the household survey component.
Specifically, $|\mathcal{J}_i| = 2$ such that for $\theta \in \{\rho, \alpha\}$ then
$$
...
If possible $k \geq 3$ is prefered, though the number of grid points scales exponentially with choice of $k$.
3. Use this quadrature rule to calculate the normalising constant $\tilde p_{\text{AQ}}(\y)$ as follows
$$
\tilde p_{\text{AQ}}(\y) = \sum_{\z \in \mathcal{Q}(m, k)} \tilde p_\text{LA}(\btheta(\z), \y) \omega(\z).
$$
4. For $i \in [n]$ generate $l$ nodes $\mathcal{X}_i$ via a Gauss-Hermite quadrature rule with one-dimension adapted based on the mode $\hat \x(\btheta)_i$ and standard deviation $\sqrt{\text{diag}[\mathbf{H}(\btheta)^{-1}]_i}$. A value of $l \geq 4$ is recommended to enable B-spline interpolation.
5. For $x_i \in \mathcal{X}_i$ and $\btheta \in \{ \btheta(\z) \}_{\z \in \mathcal{Q}(m, k)}$ calculate the modes and Hessians
\begin{align*}
\hat \x_{-i}(x_i, \btheta) &= \argmin_{\x_{-i}} - \log p(\y, x_i, \x_{-i}, \btheta), \\
\mathbf{H}_{-i, -i}(x_i, \btheta) &= \frac{\partial^2}{\partial \x_{-i} \partial \x_{-i}^\top} - \log p(\y, x_i, \x_{-i}, \btheta) \rvert_{\x_{-i} = \hat \x_{-i}(x_i, \btheta)},
\end{align*}
where optimisation to obtain $\hat \x_{-i}(x_i, \btheta)$ is initialised at $\hat \x(\btheta)_{-i}$.
6. For $x_i \in \mathcal{X}_i$ calculate
$$
\tilde p_\text{AQ}(x_i \, | \, \y) = \frac{\tilde p_\text{LA}(x_i, \y)}{p_{\text{AQ}}(\y)}.
$$
where
...
Using one `TMB` template (available in the appendix), we fit the simplified Naomi model to data from Malawi using four inferential approaches: (1) EB combined with a Gaussian approximation using `TMB`, (2) AGHQ combined with a Gaussian approximation using `aghq`, (3) AGHQ combined with a Laplace approximation by extending `aghq` and (4) the Hamiltonian Monte Carlo (HMC) algorithm No-U-Turn Sampling (NUTS) using `tmbstan`.
Details regarding particular settings used for each inferential method are provided in Table \ref{tab:inference-methods}.
