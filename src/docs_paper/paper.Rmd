...
<!-- Relevant means e.g. for alpha it's only the HIV positive individuals -->
...
This sum is over both $i \in I$ and the number of ART clients travelling from district $x'$ to $x$.
...
This requried chains of length 20,000, thinned by a factor of 20 for ease-of-storage.
...
Overlaying the generated PCA-AGHQ nodes overlaid onto the hyperparameter marginal posteriors obtained using NUTS, we found that approximately 12 of the 24 hyperparameters had well covered marginals (Figure \ref{fig:node-positions}).
...
We compare the KS statistics $D_\text{TMB} = \sup_\varphi | F_\text{NUTS}(\varphi) - F_\text{TMB}(\varphi)|$ and $D_\text{PCA-AGHQ} = \sup_\varphi | F_\text{NUTS}(\varphi) - F_\text{PCA-AGHQ}(\varphi)|$.
...
Alongside marginal distributions, we are interested in assessing the accuracy of joint distributions.
...
Joint distributions can also be compared via the maximum mean discrepancy [MMD; @gretton2006kernel].
Let $\Phi = \{\bphi_i\}_{i = 1}^n$ and $\Psi = \{\bpsi_i\}_{i = 1}^n$ be two sets of joint posterior samples obtained from different inference methods, and $k$ be a kernel.
...
In the exceedance probabilities case study (Section \ref{sec:exceedance}) we found that posterior inferences from the approximate methods were systematically inaccurate, and have the potential to meaningfully mislead policy.

PCA-AGHQ was flexibly implemented, building on the `TMB` and `aghq` \textsc{R} packages.
...
Part of our motivation for developing PCA-AGHQ was to place more quadrature nodes, and as a result computation, in those dimensions which are more important, in some sense.
While PCA is a sensible approach, there are avenues where it overlooks potential benefits, or does not behave as one might hope.
First, we identified a challenge using PCA when the dimensions have different scales, here specifically log and logit.
Second, the amount of variation explained for the Hessian matrix is not directly what we care about, rather the effect of the different dimensions on the outputs of interest.
Instead, using measures of importance from sensitivity analysis, such as Shapley values [@shapley1953value] may be preferable.
...
A potential approach could be based measuring fit using PSIS, or similar algorithms, with samples from a cheap approximation, like TMB.
This particular approach would not work however, as PSIS operates on joint distributions rather than marginals [@yao2018yes].

Other MCMC algorithms, such as blocked Gibbs sampling [@geman1984stochastic] or slice sampling [@neal2003slice], may be better suited than NUTS to sampling from Naomi.
...
The recently released version 1.0.0 of `NIMBLE` also includes functionality for automatic differentiation, Laplace approximation and HMC, built using `CppAD` as with `TMB`.
As such, it would be possible to implement AGHQ within `NIMBLE`, would facilitating access by a broader user-base.
Similarly, AGHQ could be implemented in Stan by use of the `bridgestan` package [@bridgestan] together with the adjoint-differentiated Laplace approximation of @margossian2020hamiltonian.
...
Statistical theory for PCA-AGHQ could be established by extension of Theorem 1 in @stringer2022fast.
