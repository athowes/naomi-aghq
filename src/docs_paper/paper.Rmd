...
All potential scale reduction factors [@gelman1992inference; @vehtari2021rank] attained $\hat R < 1.05$.
For full details see Appendix S2.
On this basis, though inaccuracies were still possible, we considered the NUTS results to be a gold-standard.

## PCA-AGHQ settings \label{sec:pca-settings}

We used a Scree plot (Figure \ref{fig:scree}) based on the spectral decomposition of $\hat{\Hb}_\texttt{LA}(\hat{\btheta}_\texttt{LA})$ to select the number of principal components $s < m$ to keep.
We choose $s = 8$, which was sufficient to explain close to 90% of total variation.
Reconstruction of the Hessian using this reduced rank approximation is shown in Figure \ref{fig:reduced-rank}.

```{r scree, fig.cap="Most variation could be explained by fewer than the full 24 dimensions."}
knitr::include_graphics("depends/tv-plot.png")
```

```{r reduced-rank, fig.cap="The reduced rank approximation to the Hessian is visually similar."}
...
```{r marginal-sd, fig.cap="Hyperparameters on the logit-scale had higher marginal standard deviations than those on the log-scale."}
knitr::include_graphics("depends/marginal-sd.png")
```

### Visual assessment
 
Figure \ref{fig:node-positions} shows the positions of the generated PCA-AGHQ nodes overlaid onto the hyperparameter marginal posteriors obtained using NUTS.
For approximately 12 of the 24 hyperparameters, the marginal was well covered by the nodes.
Although this is an improvement in so far as $12 > s = 8$ there do remain many hyperparameters uncovered.
Coverage was positively associated with marginal standard deviation (Figure \ref{fig:marginal-sd}), which varied particuarly according to the scale of the hyperparameter.
...
Appendix S3 shows those values which were we could compute in a reasonable time.
...
UNAIDS have developed ambitious fast-track targets for scaling up ART treatment with the goal of "ending the AIDS epidemic by 2030".
Specifically, the "90-90-90" fast-track target requires that 90% of PLHIV know their status, 90% of those are on ART, and 90% of those have suppressed viral load.
...
We found that both TMB and PCA-AGHQ underestimate these exceedance probabilities, specifically for women (Figure \ref{fig:second90}).
We hypothesise that this discrepancy is related to interactions between the household survey and ANC components of the model creating a more challenging posterior geometry.
...
We are yet to formulate hypothesis as to why this is the case.
...
Among the ELGM-type structures of particular interest in spatial epidemiology and feature in Naomi, are aggregated Gaussian process models [@nandi2020disaggregation] and evidence synthesis models [@amoah2020geostatistical].
...
Hyperparameters with skewed marginal posteriors are more important to cover well using quadrature nodes.
