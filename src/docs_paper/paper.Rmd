...
The dimension of the latent field is $N = 467$ and the dimension of the hyperparameters is $m = 24$.
...
Let $\{\phi_i\}_{i = 1}^n$ be posterior marginal samples from some quantity with empirical cumulative distribution (ECDF) function $F(\varphi) = \frac{1}{n} \sum_{i = 1}^n \mathbb{I}_{\phi_i \leq \varphi}$.
...
We are interested in assessing the accuracy of joint distributions as well as marginal distributions.
Let $\{\bphi_i\}_{i = 1}^n$ be posterior joint samples.
Pareto smoothed importance sampling (PSIS) [@vehtari2015pareto, @yao2018yes].
...
## Case study on exceedance probabilites

### Meeting the second 90

The Joint United Nations Programme on HIV/AIDS (UNAIDS) has developed ambitious fast-track targets for scaling up ART treatment with the goal to "end the AIDS epidemic by 2030".
The 90-90-90 by 2020 fast-track target is that 90% of PLHIV know their status, 90% of those are on antiretroviral therapy (ART) and 90% of those have suppressed viral load.
Naomi can be used to assess the probability that the second 90 target has been met, thereby helping to find treatment gaps, by evaluating $\mathbb{P}(\alpha_i > 0.81)$ for each strata $i$.
See a summary of the results in Figure \ref{fig:second90}.

```{r second90, fig.cap="Results of second 90 case-study."}
knitr::include_graphics("depends/second90.png")
```

### Finding strata with high incidence strata

Some HIV interventions are cost-effective only within high HIV incidence settings, typically defined as above 1% incidence per year.
Naomi can be used to assess the probability of there being high incidence within a strata by evaluating $\mathbb{P}(\lambda_i > 0.01)$.
See a summary of the results in Figure \ref{fig:1inc}.
