...
We consider a simplified version of the full model which is defined only at $T_1$ and omits temporal projection.
...
Let $i$ index the coarsest district-age-sex division included in the model.
...
We specify independent mixed effects models for HIV prevalence and ART coverage on the logit scale such that
\begin{align}
\text{logit}(\rho_i) &= \eta^\rho_i, \\
\text{logit}(\alpha_i) &= \eta^\alpha_i,
\end{align}
for certain choice of linear predictors $\eta^\rho_i$ and $\eta^\alpha_i$.
For the HIV incidence rate we use a mixed effects model on the log scale where the linear predictor depends on $\{\rho_i, \alpha_i\}_{i \in \mathcal{I}}$
$$
\log(\lambda_i) = \eta^\rho_i(\{\rho_i, \alpha_i\}_{i \in \mathcal{I}}).
$$

## Likelihood

Consider a household survey occurring at $T_1$.
Let $\mathcal{I}$ be a set of $i$ for which observations are reported.
For each $\mathcal{I}$, we calculate the weighted observations $\hat \theta_\mathcal{I}$ for $\theta \in \{\rho, \alpha, \kappa\}$ with respective Kish effective sample sizes $M^{\hat \theta}_\mathcal{I}$, and observed number of cases
$$
