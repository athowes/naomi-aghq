...
<!-- * [ ] Description of the ART attendance component of the model -->
...
4. In all components the data we model is aggregated. Processes are modelled at the finest distict-age-sex division $i$, but likelihoods are defined for observations over sets of indices $\mathcal{I}$.

<!-- To-do: define what exactly $p(\y, \x, \btheta)$ is for the Naomi model -->
...
A quadrature rule can be used to approximate the integral of a function $f: \mathcal{Z} \to \mathbb{R}$ by a weighted sum
$$
...
In Gauss-Hermite quadrature [@davis1975methods] the nodes are selected as zeros of the Hermite polynomials.
