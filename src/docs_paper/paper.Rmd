...
A more complete mathematical description, as well as a \textsf{C++} template for the log-posterior, are provided in the appendix.
...
We model the log-odds $\tilde \gamma_{x, x'} = \text{logit}(\gamma_{x, x'})$ using a multinomial logistic regression model.

This section remains to write, and is quite challenging.
...
\end{align}
Given the negative unnormalised log posterior $- \log p(\y, \x, \btheta)$, we obtain the above posterior marginal approximations $\{ \tilde p(x_i \, | \, \y) \}_{i = 1}^n$ and $\tilde p(\theta_j \, | \, \y)_{j = 1}^m$ via nested applications of the Laplace approximation and AGHQ, described below.
