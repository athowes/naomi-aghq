...
Given a \textsf{C++} user template `model.cpp` for the unnormalised log posterior, we obtain the posterior marginal approximations given in Equations \eqref{eq:inla1} and \eqref{eq:inla2} via the following algorithm:
...
$$
The nodes and weights $\{\mathcal{Q}(m, k_\theta), \omega_\theta\}$ used to obtain $\tilde p_{\text{AQ}}(\y)$ are reused to perform integration with respect to the hyperparameters above.
For each of the $k_\theta^m$ values of $\btheta(\z_\theta)$ we obtain $\tilde p_\text{AQ}(x_i \, | \, \btheta(\z_\theta), \y)$ by setting $\btheta = \btheta(\z_\theta)$ in the following two steps.
