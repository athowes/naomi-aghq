...
An overview of the simplified model is given below, highlighting the aspects which make it a challenge for existing inferential approaches.
...
We use a binomial working likelihood $y^{\hat \theta}_{\mathcal{I}} \sim \text{xBin}(m^{\hat \theta}_{\mathcal{I}}, \theta_{\mathcal{I}})$ to model these aggregate observations, where $\theta_{\mathcal{I}}$ are the following appropriately weighted aggregates
\begin{equation*}
...
Given the negative unnormalised log posterior $- \log p(\y, \x, \btheta)$, we obtain the posterior marginal approximations $\{ \tilde p(x_i \, | \, \y) \}_{i = 1}^n$ and $\tilde p(\theta_j \, | \, \y)_{j = 1}^m$ via the following algorithm, comprised of nested applications of the Laplace approximation and AGHQ.
...
\end{align*}
2. Generate a set of nodes $\bu \in \mathcal{Q}(m, k)$ and weights $\omega: \bu \in \mathcal{Q}(m, k) \to \mathbb{R}$ from a Gauss-Hermite quadrature rule with $k$ nodes per dimension, which are then adapted based on the mode and lower Choleksy via $\btheta(\bu) = \hat{\btheta} + \mathbf{L} \bu$.
...
For all methods, we simulated age-sex-district specific HIV prevalence, ART coverage and HIV incidence.
...
MCMC can be used to obtain accurate inferential results only once convergence has been reached and the chain lengths are sufficient.
...
Our method is demonstrated to be more accurate than the EB Gaussian approximation currently in use, and substantially faster than NUTS for the simplified Naomi model in Malawi (Section \ref{sec:results}).
We anticipate that our method could be added to the Naomi software as an alternative option to `TMB`.
...
Future work could look to implement our algorithm (Section \ref{sec:algorithm}) into probabilistic programming languages, facilitating access by a broader user-base.
