...
In addition, we require the inference procedure to be automatic and produce estimates reliably, across a range of country settings, with specific data not known in advance.
As such, approaches requiring substantial statistical expertise, as would be the case for monitoring MCMC convergence and suitability, are not viable.
...
We expected this would result in underestimation of posterior variances, which may have practical implications for use of the resulting estimates.
...
To meet these requirements, we developed an inference method based on adaptive Gauss-Hermite quadrature (AGHQ) extended to handle integration over large numbers of hyperparameters.
