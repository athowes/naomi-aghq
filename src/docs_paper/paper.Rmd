...
Although the approach of `TMB` is fast, within the empirical Bayes framework hyperparameter uncertainty is not accounted for in the latent field posterior.
...
We developed an inference method based on adaptive Gauss-Hermite quadrature (AGHQ) extended to handle integration over many hyperparameters.
...
The latent field is controlled by hyperparamters $\btheta$ which include standard deviations, first-order autoregressive model correlation parameters, and reparameterised Besag-York-Mollie model [@simpson2017penalising] proportion parameters.
...
Supposing the hyperparameters are to be considered fixed, then latent field joint and marginal inferences then follow directly from the Gaussian approximation $\tilde p_\texttt{G}(\x \, | \, \hat{\btheta}_\texttt{LA}, \y)$.
This is the approach used currently for Naomi, which we refer to as TMB (distinguished from the software \texttt{TMB}).
...
Gauss-Hermite quadrature [GHQ; @davis1975methods] is a quadrature rule, where the nodes are selected as zeros of the $k$th Hermite polynomial
\begin{equation}
...
We term this approach PCA-AGHQ, with estimate of the normalising constant given by
\begin{equation}
...
The final panel of Figure \ref{fig:aghq} illustrates PCA-AGHQ for a case when $m = 2$ and $s = 1$. 
...
1. TMB (`r signif(time_taken$TMB, 2)` seconds), using a Gaussian approximation at $\hat{\btheta}_\texttt{LA}$.
2. PCA-AGHQ (`r signif(time_taken$aghq, 2)` hours), using a mixture of Gaussians approximation at a PCA-AGHQ quadrature grid, as described in Section \ref{sec:pca}, and implemented via extension of the `aghq` package [@stringer2021implementing].
...
All potential scale reduction factors [@gelman1992inference; @vehtari2021rank] were $\hat R < 1.05$.
...
We used a Scree plot (Figure S2) based on the spectral decomposition of $\hat{\Hb}_\texttt{LA}(\hat{\btheta}_\texttt{LA})^{-1}$ to select the number of principal components to keep.
We found that $s = 8$ principal components were sufficient to explain close to 90% of total variation.
With this choice of $s$, the reduced rank approximation to the inverse curvature was visually similar (Figure S3).
...
The ordered eigenvectors can be interpreted to inform us about the directions of greatest variation in the inverse curvature (Figure S6)
The first and second eigenvectors are the weakly identified AR1 standard deviation and correlation parameters.
Variability can be explained both by low correlation or by high standard deviation.
