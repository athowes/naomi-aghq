...
Accurate estimates of HIV indicators are required to mount effective public health response to the epidemic.
The Naomi small-area estimation model [@eaton2021naomi] synthesises data from multiple sources to estimate HIV prevalence, HIV incidence, and coverage of antiretroviral treatment (ART) at a district-level.
Software has been developed for the model allowing countries to use their data to generate estimates in a yearly process supported UNAIDS.
As such, the inference method used must be suitable to run in production, ruling out prohibitively slow Markov chain Monte Carlo approaches.
...
Instead, inference is conducted using the Template Model Builder (`TMB`) \textsf{R} package [@kristensen2016tmb], which implements a variety of empirical Bayes inference, and is gaining popularity in spatial statistics as a flexible alternative to `R-INLA` [@osgoodzimmerman2021statistical].

# Background

## Integrated nested Laplace approximation \label{sec:inla}

INLA is an approximate Bayesian inference method based on the Laplace approximation and numerical integration.
...
\end{align}
An approximation is made to each of the two quantities, $p(\btheta \, | \, \y)$ and $p(x_i \, | \, \btheta, \y)$, nested inside the above integrals: (i) $p(\btheta \, | \, \y) \approx \tilde p(\btheta \, | \, \y)$ and (ii) $p(x_i \, | \, \btheta, \y) \approx \tilde p(x_i \, | \, \btheta, \y)$, which we discuss in turn below.
...
\end{equation}
Approximation (i) then uses a Gaussian approximation to the denominator of Equation \ref{eq:hyperpost} given by
\begin{equation}
...
\end{equation}
close to being Gaussian because $\y$ is generally not that informative and the observation distribution $p(\y \, | \, \x, \btheta)$ is usually well-behaved [@blangiardo2015spatial].
...
\end{equation}
where the marginal mean $\hat{\mu}_i(\btheta)$ and precision $\hat{q}_i(\btheta)$ are recovered directly from $\hat{\bm{\mu}}(\btheta)$ and $\hat{\bm{Q}}(\btheta))$ respectively.
...
\end{align}
where $p_G(\x_{-i} \, | \, x_i, \btheta, \y)$ is the Gaussian approximation to $\x_{-i} \, | \, x_i, \btheta, \y$ and $\hat{\bm{\mu}}_{-i}(x_i, \btheta)$ is its modal configuration.\footnote{Note that $\hat{\bm{\mu}}(\btheta)$ is the mode of the Gaussian approximation to the full latent field given $\btheta$ and $\hat{\bm{\mu}}_{-i}(x_i, \btheta)$ is not the same as $\hat{\bm{\mu}}_{-i}(\btheta)$.}
...
The approach of `TMB` is inspired by the AD Model Builder (ADMB) package [@fournier2012ad].
...
AD is popular in machine learning [@baydin2017automatic], for example as the basis for backpropagation algorithm and is beginning to gain popularity in statistics, including as a part of Stan [@carpenter2017stan].
