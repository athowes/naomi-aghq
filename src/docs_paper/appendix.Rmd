...
```{r rhat, fig.cap="The potential scale reduction factor compared between- and within- estimates of univariate parameters. It is recommended against to use the results if the value is less than 1.05."}
knitr::include_graphics("depends/rhat.png")
```

```{r ratio, fig.cap="The ESS ratio measures how efficient sampling is."}
knitr::include_graphics("depends/ratio.png")
```

```{r ess, fig.cap="The effective number of samples varied substantially."}
...
```{r reduced-rank, fig.cap="The reduced rank (8) matrix approximation to the Hessian is visually similar to the full rank (24) matrix."}
...
In earlier work, we used a full Laplace approximation to the latent field marginals.
The algorithm for obtaining this approximation is detailed here.
...
$l \geq 4$ is recommended to enable B-spline interpolation.
