...
In this section we describe the simplified Naomi model considered in the main text in greater detail.
In doing so we draw from the supplementary material of @eaton2021naomi.
...
We use use structured random effects to enable partial pooling of information across units assessed as being similar, such as neighbouring districts or adjacent age-bands.
Let $u$ be a generic random effect, with some length.
...
In the case of Malawi then $n = 32$ such that the total number of latent field parameters is $499$ and the total number of hyperparameters is $24$.
...
The two BYM2 random effects are comprised of the following unit scaled patially structured $\{v_x^{\rho}, v_x^{\rho, s = \text{M}}\}$ and spatially unstructured $\{w_x^{\rho}, w_x^{\rho, s = \text{M}}\}$ components
\begin{align*}
...
\end{align*}
We use half-normal priors for the standard deviation terms $\{\sigma_A^\rho, \sigma_{AS}^\rho, \sigma_X^\rho, \sigma_{XS}^\rho, \sigma_{XA}^\rho\} \sim \mathcal{N}^{+}(0, 2.5)$, uniform priors for the AR1 correlation parameters $\{\phi_A^\rho, \phi_{AS}^\rho\} \sim \mathcal{U}(-1, 1)$, and beta priors for the BYM2 proportion parameters $\{\phi_X^\rho, \phi_{XS}^\rho\} \sim \text{Beta}(0.5, 0.5)$.
...
1. If survey data on HIV prevalence or ART coverage by age and sex are not available then we set $u_a^\theta = 0$ and $u_{a, s = \text{M}}^\theta = 0$ and use the average age-sex pattern of from the Spectrum offset $\eta_{R_x, s, a}^\theta$. For the Malawi example considered in the main text HIV prevalence and ART coverage data is not available for those aged 65+.
...
\texttt{u\_rho\_x} & $w^\rho_x$ & Latent & $n$ & $\mathbb{R}^{n}$ \\
\texttt{us\_rho\_x} & $v^\rho_x$ & Latent & $n$ & $\mathbb{R}^{n}$ \\
\texttt{u\_rho\_xs} & $w_x^{\rho, s = \text{M}}$ & Latent & $n$ & $\mathbb{R}^{n}$ \\
\texttt{us\_rho\_xs} & $v_x^{\rho, s = \text{M}}$ & Latent & $n$ & $\mathbb{R}^{n}$ \\
...
\texttt{u\_alpha\_x} & $w^\alpha_x$ & Latent & $n$ & $\mathbb{R}^{n}$ \\
\texttt{us\_alpha\_x} & $v^\alpha_x$ & Latent & $n$ & $\mathbb{R}^{n}$ \\
\texttt{u\_alpha\_xs} & $w_x^{\alpha, s = \text{M}}$ & Latent & $n$ & $\mathbb{R}^{n}$ \\
\texttt{us\_alpha\_xs} & $v_x^{\alpha, s = \text{M}}$ & Latent & $n$ & $\mathbb{R}^{n}$ \\
...
\caption{Correspondence between mathematical notation and variable names used in our \texttt{TMB} code. The total number of hyperparameters is 24, and the total number of latent field parameters is $51 + 14n$, where $n$ is the number of districts.}
\end{table}
```
