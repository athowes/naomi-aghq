...
Let $x$ be a district, located within the Spectrum region $R_x$, $s \in \{\text{F}, \text{M}\}$ be sex, and $a \in \{\text{0-5}, \text{5-10}, \ldots, \text{75-80}, 80+\}$ be five-year age bands.
Consider the time period $t = T_1$ of the most recent national household survey with HIV testing specified to the nearest calendar quarter.
We index population size $N_{x, s, a, t}$, HIV prevalence $\rho_{x, s, a, t}$, ART coverage $\alpha_{x, s, a, t}$ and annual HIV incidence rate $\lambda_{x, s, a, t}$ by district, sex, age-band and time period.

## Structured random effects

We use $u \sim \text{ICAR}(\sigma)$ to refer to the Besag intrinsic conditional auto-regressive model (ICAR) with marginal standard deviation $\sigma > 0$, and $u \sim \text{BYM}2(\sigma, \phi)$ to the reparameterised Besag-York-Mollie model (BYM2) [@simpson2017penalising] with marginal standard deviation $\sigma > 0$ comprised of a spatially structured ICAR component with proportion $\phi \in (0, 1)$ and spatially unstructured IID component with proportion $1 - \phi$.
For both the ICAR and BYM2 models, we follow recommendations [@freni2018note] on scaling, disconnected adjacency graph components, and islands.
...
$$
where $\beta^\rho_0$ is the intercept, $\beta_{s}^{\rho, s = \text{M}}$ is the difference in logit prevalence for men compared to women, $u^\rho_a \sim \text{AR}1(\sigma_A^\rho, \phi_A^\rho)$ are age random effects for women, $u_a^{\rho, s = \text{M}}$ are age random effects for the difference in logit prevalence for men compared to women age $a$, $u^\rho_x \sim \text{BYM}2(\sigma_X^\rho, \phi_X^\rho)$ are spatial random effects for women, $u_x^{\rho, s = \text{M}} \sim \text{BYM}2(\sigma_{XS}^\rho, \phi_{XS}^\rho)$ are spatial random effects for the difference in logit prevalence for men compared to women in district $x$, $u_x^{\rho, a < 15} \sim \text{ICAR}(0, \sigma_{XA}^\rho)$ are spatial random effects for the ratio of paediatric prevalence to adult women prevalence, and $\eta^\rho_{R_x, s, a}$ are a fixed offset specifying assumed odds ratios for prevalence outside the age ranges for which data are available.
We use the prior distributions $\mathcal{N}(0, 5)$ for all fixed effects ($\beta^\rho_0, \beta_{s}^{\rho, s = \text{M}}$), $\mathcal{N}^{+}(0, 2.5)$ for all standard deviation terms, $\mathcal{U}(-1, 1)$ for all AR1 correlation parameters, and $\text{Beta}(0.5, 0.5)$ for all BYM2 proportion parameters.
...
$$
with analogous terms to the HIV prevalence model.
...
$$
where $\beta^\lambda_0$ is the intercept, which is proportional to the average HIV transmission rate for untreated HIV positive adults, $\beta_S^{\lambda, s = \text{M}}$ is the log incidence rate ratio for men compared to women, $\rho_{x, t}^{\text{15-49}}$ is the HIV prevalence among adults 15-49 calculated by
$$
...
$$
where $N_{x, \text{F}, a, t}$ are the female population sizes, $\psi_{R_x, a, t}$ are ASFR in Spectrum region $R_x$ at time $t$, $\beta^\psi$ are the log rate ratio for the number of ANC clients relative to the predicted fertility, $u_x^\psi \sim \mathcal{N}(0, \sigma^\psi)$ are district random effects, HIV prevalence $\rho_{x, a, t}^\text{ANC}$ and ART coverage $\alpha_{x, a, t}^\text{ANC}$ among pregnant women modelled with logit-linear models
\begin{align}
