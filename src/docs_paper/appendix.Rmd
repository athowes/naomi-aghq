...
Here, we describe the simplified Naomi model [@eaton2021naomi] in more detail.
...
We then use $\{\cdot\}$ to generically refer to a aggregate set over which an observation is made, e.g. $\{a\} = \{\text{15-19}, \ldots, \text{45-49}\}$ for the adult age range 15-49.
Let $\sum_{\{x\}}$ be shorthand for $\sum_{x \in \{x\}}$, analogously for $s$ and $a$.

### Structured random effects

We use structured random effects to enable partial pooling of information across units assessed as being similar, such as those belonging to neighbouring districts or adjacent age-bands.
...
Let $w_j = 1 / \pi_j \times 1 / \omega_j$ be design weights, where $\omega_j$ is a non-response factor, then a survey weighted mean, calculated using the `survey` \textsc{R} package [@lumley2004analysis], is given by
$$
...
\begin{equation}
\text{logit}(\rho_{x, s, a}) = \beta^\rho_0 + \beta_{S}^{\rho, s = \text{M}} + u^\rho_a + u_a^{\rho, s = \text{M}} + u^\rho_x + u_x^{\rho, s = \text{M}} + u_x^{\rho, a < 15} + \eta^\rho_{R_x, s, a}. \label{eq:prev}
\end{equation}
...
$u^\rho_a$ & $\text{AR}1(\sigma_A^\rho, \phi_A^\rho)$ & Age random effects for women \\
$u_a^{\rho, s = \text{M}}$ & $\text{AR}1(\sigma_{AS}^\rho, \phi_{AS}^\rho)$ & Age random effects for the difference in logit prevalence for men compared to women age $a$ \\
$u^\rho_x$ & $\text{BYM}2(\sigma_X^\rho, \phi_X^\rho)$ & Spatial random effects for women \\
$u_x^{\rho, s = \text{M}}$ & $\text{BYM}2(\sigma_{XS}^\rho, \phi_{XS}^\rho)$ & Spatial random effects for the difference in logit prevalence for men compared to women in district $x$  \\
...
$\eta^\rho_{R_x, s, a}$ & $-$ & Fixed offsets specifying assumed odds ratios for prevalence outside the age ranges for which data are available \\
\bottomrule
\end{tabularx}
...
The two BYM2 random effects $u^\rho_x$ and $u_x^{\rho, s = \text{M}}$ are comprised of the following unit scaled spatially structured $\{v_x^{\rho}, v_x^{\rho, s = \text{M}}\}$ and spatially unstructured $\{w_x^{\rho}, w_x^{\rho, s = \text{M}}\}$ components, respectively
\begin{align*}
...
\begin{equation}
\log(\lambda_{x, s, a}) = \beta_0^\lambda + \beta_S^{\lambda, s = \text{M}} + \log(\rho_{x}^{\text{15-49}}) + \log(1 - \omega \cdot \alpha_{x}^{\text{15-49}}) + u_x^\lambda + \eta_{R_x, s, a}^\lambda. \label{eq:inc}
\end{equation}
...
\begin{align*}
y^{\rho}_{\{x\}, \{s\}, \{a\}} &\sim \text{xBin}(m^{\rho}_{\{x\}, \{s\}, \{a\}}, \rho_{\{x\}, \{s\}, \{a\}}), \quad
\rho_{\{x\}, \{s\}, \{a\}} = \frac{\sum_{\{x\}} \sum_{\{s\}} \sum_{\{a\}} N_{x, s, a} \cdot \rho_{x, s, a}}{\sum_{\{x\}} \sum_{\{s\}} \sum_{\{a\}} N_{x, s, a}}, \\
y^{\alpha}_{\{x\}, \{s\}, \{a\}} &\sim \text{xBin}(m^{\alpha}_{\{x\}, \{s\}, \{a\}}, \alpha_{\{x\}, \{s\}, \{a\}}), \quad
...
y^{\kappa}_{\{x\}, \{s\}, \{a\}} &\sim \text{xBin}(m^{\kappa}_{\{x\}, \{s\}, \{a\}}, \kappa_{\{x\}, \{s\}, \{a\}}), \quad
...
The C++ `TMB` code for the negative log-posterior of the simplified Naomi model is available from the GitHub repository `athowes/elgm-inf`.
...
```{r scree, fig.cap="Most variation could be explained by far fewer than the full 24 dimensions."}
knitr::include_graphics("depends/tv-plot.png")
```

```{r reduced-rank, fig.cap="The reduced rank (8) matrix approximation to the Hessian is visually similar to the full rank matrix."}
...
# Further inference comparison results
