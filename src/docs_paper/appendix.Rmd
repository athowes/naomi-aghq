...
In this section we describe the simplified Naomi model [@eaton2021naomi] in more complete mathematical detail than the main text.
...
The notation $\sum_{\{x\}}$ is used as shorthand for $\sum_{x \in \{x\}}$, and analogously for $s$ and $a$.
...
```{r pc-loadings, fig.cap="Principal component loadings."}
knitr::include_graphics("depends/pc-loadings.png")
```

```{r reduced-rank, fig.cap="The reduced rank (8) matrix approximation to the covariance matrix is visually similar to the full rank original."}
...
ks_summary %>%
  ...
  select(-type) %>%
  rename(`PCA-AGHQ` = `KS(aghq, tmbstan)`, `TMB` = `KS(TMB, tmbstan)`) %>%
  gt::gt(rowname_col = "Parameter") %>%
  gt::fmt_number(
    columns = c(`PCA-AGHQ`, `TMB`),
    decimals = 3
  ) %>%
  gt::summary_rows(
   columns = everything(),
...
```{r rho_a, fig.cap="Variation between units can be explained either by high correlation and high variance, or by low correlation and low variance. As such, some hyperparameters has correlated posteriors. It is this correlation which is made use of by PCA-AGHQ."}
