...
In this section we describe more fully the simplified Naomi model considered in the main text. In doing so we draw from the supplementary material of @eaton2021naomi.

## Background

### Indexing

Let $\nu$ the most recent national household survey with HIV testing which has taken place in the country of interest.
...
Finally, we let $\sum_{\{x\}}$ be used as shorthand for $\sum_{x \in \{x\}}$, and likewise for $s$ and $a$.

### Structured random effects

Let $u$ be a generic random effect. We use use structured random effects to enable partial pooling of information across units assessed as being similar, such as neighbouring districts or adjacent age-bands. We specify the first order auto-regressive model by $u \sim \text{AR}1(\sigma, \phi)$ such that
\begin{align*}
u_1 &\sim \left( 0, \frac{1}{1 - \rho^2} \right), \\
u_i &= \rho u_{i - 1} + \epsilon_t, \quad i = 2, \ldots
\end{align*}
where $\epsilon_i \sim \mathcal{N}(0, 1)$ is Gaussian white noise, and $|\rho| < 1$ is the lag-one correlation parameter.
...
We assume the household survey was run according to a complex survey design where each individual $j \in U$ has non-zero probability $\pi_j \in (0, 1)$ of appearing in the sample $S \subseteq U$. Suppose we observe an outcome $y_j$ for $j \in S$.
...
$$
m = \frac{\left(\sum_{j \in S} w_j\right)^2}{\sum_{j \in S} w_j^2}.
$$
...
\toprule
& Model section & Latent field & Hyperparameter \\ 
\midrule
\ref{sec:hiv-prev} & HIV prevalence & $36 + 3n$ & 9 \\ 
\ref{sec:art-cov} & ART coverage & $36 + 3n$ & 9 \\ 
\ref{sec:hiv-inc} & HIV incidence rate & $2 + n$ & 3 \\ 
\ref{sec:anc-test} & ANC testing & $4 + 2n$ & 2 \\ 
...
In this section, we describe the hyperparameter and latent field process specification for the model.
Whereas in the main text process and likelihood specifications are written together, here we consider the likelihood equations separately in Section \ref{sec:likelihood}.
Table \ref{tab:latent-hyper-size} gives the number of latent field parameters and hyperparameters in each section of the model.
Given a country with $n$ districts, then the total number of unique district-age-sex combinations is $n \times 2 \times 17 = 34n$.
In the case of Malawi then $n = 32$ such that the total number of latent field parameters is $491$ and the total number of hyperparameters is $24$.
...
$$
where $\beta^\rho_0 \sim \mathcal{N}(0, 5)$ is an intercept term, $\beta_{s}^{\rho, s = \text{M}} \sim \mathcal{N}(0, 5)$ is the difference in logit prevalence for men compared to women, $u^\rho_a \sim \text{AR}1(\sigma_A^\rho, \phi_A^\rho)$ are age random effects for women, $u_a^{\rho, s = \text{M}} \sim \text{AR}1(\sigma_{AS}^\rho, \phi_{AS}^\rho)$ are age random effects for the difference in logit prevalence for men compared to women age $a$, $u^\rho_x \sim \text{BYM}2(\sigma_X^\rho, \phi_X^\rho)$ are spatial random effects for women, $u_x^{\rho, s = \text{M}} \sim \text{BYM}2(\sigma_{XS}^\rho, \phi_{XS}^\rho)$ are spatial random effects for the difference in logit prevalence for men compared to women in district $x$, $u_x^{\rho, a < 15} \sim \text{ICAR}(\sigma_{XA}^\rho)$ are spatial random effects for the ratio of paediatric prevalence to adult women prevalence, and $\eta^\rho_{R_x, s, a}$ are fixed offsets specifying assumed odds ratios for prevalence outside the age ranges for which data are available.
We use the prior distributions $\mathcal{N}^{+}(0, 2.5)$ for the standard deviation terms $\{\sigma_A^\rho, \sigma_{AS}^\rho, \sigma_X^\rho, \sigma_{XS}^\rho, \sigma_{XA}^\rho\}$, $\mathcal{U}(-1, 1)$ for the AR1 correlation parameters $\{\phi_A^\rho, \phi_{AS}^\rho\}$, and $\text{Beta}(0.5, 0.5)$ for the BYM2 proportion parameters $\{\phi_X^\rho, \phi_{XS}^\rho\}$.

<!-- Latent field size -->

<!-- 1 + 1 + 17 + 17 + n + n + n = 36 + 3n -->
...
$$
where $\beta^\lambda_0 \sim \mathcal{N}(0, 5)$ is an intercept term proportional to the average HIV transmission rate for untreated HIV positive adults, $\beta_S^{\lambda, s = \text{M}} \sim \mathcal{N}(0, 5)$ is the log incidence rate ratio for men compared to women,
$$
...
The term $\omega$ is the average reduction in HIV transmission rate per 1% increase in population ART coverage and is fixed at $\omega = 0.7$ based on inputs to the Estimation and Projection Package (EPP) model [@eaton2019estimation], $u_x^\lambda \sim \mathcal{N}(0, \sigma^\lambda)$ with $\sigma^\lambda \sim \mathcal{N}^+(0, 1)$ are IID spatial random effects, and $\eta^\lambda_{R_x, s, a}$ specify fixed log incidence rate ratios by sex and age group calculated from Spectrum model output.
...
$$
where $\Omega_T \sim \mathcal{N}(\Omega_{T_0}, \sigma^{\Omega_T})$ is the mean duration of recent infection (MDRI), and $\beta_T \sim \mathcal{N}^{+}(\beta_{T_0}, \sigma^{\beta_T})$ is the false recent ratio (FRR).
...
For adult women 15-49 we model the number of ANC clients $\Psi_{x, a} > 0$ on the log scale
$$
\log (\Psi_{x, a}) = \log (N_{x, \text{F}, a}) + \psi_{R_x, a} + \beta^\psi + u_x^\psi
$$
where $N_{x, \text{F}, a}$ are the female population sizes, $\psi_{R_x, a}$ are fixed age-sex fertility ratios in Spectrum region $R_x$, $\beta^\psi$ are fixed log rate ratios for the number of ANC clients relative to the predicted fertility, and $u_x^\psi \sim \mathcal{N}(0, \sigma^\psi)$ are district random effects, with $\sigma^\psi \sim \mathcal{N}^+(0, 1)$.
...
For HIV prevalence, ART coverage and recent HIV infections, denoted by $\theta \in \{\rho, \alpha, \kappa\}$, the household survey $\nu$ furnishes weighted observations $\hat \theta_{\{x\}, \{s\}, \{a\}}$ with respective Kish effective sample sizes $m^{\hat \theta}_{\{x\}, \{s\}, \{a\}}$, and observed number of cases $y^{\hat \theta}_{\{x\}, \{s\}, \{a\}} = m^{\hat \theta}_{\{x\}, \{s\}, \{a\}} \cdot \hat \theta_{\{x\}, \{s\}, \{a\}}$.
We use following three binomial working likelihoods
\begin{align*}
...
Let $W^\text{ANC}_{\{x\}}$ be the number of ANC clients, $X^\text{ANC}_{\{x\}}$ the number of those with ascertained status, $Y^\text{ANC}_{\{x\}}$ the number of those with positive status (either known or tested) and $Z^\text{ANC}_{\{x\}}$ the number of ANC clients already on ART prior to first ANC, such that $W^\text{ANC}_{x} \geq X^\text{ANC}_{x} \geq y^\text{ANC}_{x} \geq Z^\text{ANC}_{x}$ for all $x \in \{x\}$.
...
For the Malawi example considered in the main text, all required data are available.
...
\toprule
Variable name & Mathematical notation & Type & Dimension & Domain \\ 
\midrule
\texttt{logit\_phi\_rho\_x} & $\text{logit}(\phi_X^\rho)$ & Hyper & 1 & $\mathbb{R}$ \\
\texttt{log\_sigma\_rho\_x} & $\log(\sigma_X^\rho)$ & Hyper & 1 & $\mathbb{R}$ \\
\texttt{logit\_phi\_rho\_xs} & $\text{logit}(\phi_{XS}^\rho)$ & Hyper & 1 & $\mathbb{R}$ \\
\texttt{log\_sigma\_rho\_xs} & $\log(\sigma_{XS}^\rho)$ & Hyper & 1 & $\mathbb{R}$ \\
\texttt{logit\_phi\_rho\_a} & $\text{logit}(\phi_A^\rho)$ & Hyper & 1 & $\mathbb{R}$ \\
\texttt{log\_sigma\_rho\_a} & $\log(\sigma_A^\rho)$ & Hyper & 1 & $\mathbb{R}$ \\
\texttt{logit\_phi\_rho\_as} & $\text{logit}(\phi_{AS}^\rho)$ & Hyper & 1 & $\mathbb{R}$ \\
\texttt{log\_sigma\_rho\_as} & $\log(\sigma_{AS}^\rho)$ & Hyper & 1 & $\mathbb{R}$ \\
\texttt{log\_sigma\_rho\_xa} & $\log(\sigma_{XA}^\rho)$ & Hyper & 1 & $\mathbb{R}$ \\
\texttt{logit\_phi\_alpha\_x} & $\text{logit}(\phi_X^\alpha)$ & Hyper & 1 & $\mathbb{R}$ \\
\texttt{log\_sigma\_alpha\_x} & $\log(\sigma_X^\alpha)$ & Hyper & 1 & $\mathbb{R}$ \\
\texttt{logit\_phi\_alpha\_xs} & $\text{logit}(\phi_{XS}^\alpha)$ & Hyper & 1 & $\mathbb{R}$ \\
\texttt{log\_sigma\_alpha\_xs} & $\log(\sigma_{XS}^\alpha)$ & Hyper & 1 & $\mathbb{R}$ \\
\texttt{logit\_phi\_alpha\_a} & $\text{logit}(\phi_A^\alpha)$ & Hyper & 1 & $\mathbb{R}$ \\
\texttt{log\_sigma\_alpha\_a} & $\log(\sigma_A^\alpha)$ & Hyper & 1 & $\mathbb{R}$ \\
\texttt{logit\_phi\_alpha\_as} & $\text{logit}(\phi_{AS}^\alpha)$ & Hyper & 1 & $\mathbb{R}$ \\
\texttt{log\_sigma\_alpha\_as} & $\log(\sigma_{AS}^\alpha)$ & Hyper & 1 & $\mathbb{R}$ \\
\texttt{log\_sigma\_alpha\_xa} & $\log(\sigma_{XA}^\alpha)$ & Hyper & 1 & $\mathbb{R}$ \\
\texttt{OmegaT\_raw} & $\Omega_T$ & Hyper & 1 & $\mathbb{R}$ \\
\texttt{log\_betaT} & $\log(\beta_T)$ & Hyper & 1 & $\mathbb{R}$ \\
\texttt{log\_sigma\_lambda\_x} & $\log(\sigma^\lambda)$ & Hyper & 1 & $\mathbb{R}$ \\
...
\texttt{beta\_rho} & $(\beta^\rho_0, \beta_{s}^{\rho, s = \text{M}})$ & Latent & 2 & $\mathbb{R}^2$ \\
\texttt{beta\_alpha} & $(\beta^\alpha_0, \beta_{S}^{\alpha, s = \text{M}})$ & Latent & 2 & $\mathbb{R}^2$ \\
\texttt{beta\_lambda} & $(\beta_0^\lambda, \beta_S^{\lambda, s = \text{M}})$ & Latent & 2 & $\mathbb{R}^2$ \\
\texttt{beta\_anc\_rho} & $\beta^{\rho^{\text{ANC}}}$ & Latent & 1 & $\mathbb{R}$ \\
\texttt{beta\_anc\_alpha} & $\beta^{\alpha^{\text{ANC}}}$ & Latent & 1 & $\mathbb{R}$ \\
\texttt{u\_rho\_x} & $u^\rho_x$ & Latent & $n$ & $\mathbb{R}^{n}$ \\
\texttt{us\_rho\_x} & & Latent & $n$ & $\mathbb{R}^{n}$ \\
\texttt{u\_rho\_xs} & $u_x^{\rho, s = \text{M}}$ & Latent & $n$ & $\mathbb{R}^{n}$ \\
\texttt{us\_rho\_xs} & & Latent & $n$ & $\mathbb{R}^{n}$ \\
\texttt{u\_rho\_a} & $u^\rho_a$ & Latent & 10 & $\mathbb{R}^{10}$ \\
\texttt{u\_rho\_as} & $u_a^{\rho, s = \text{M}}$ & Latent & 10 & $\mathbb{R}^{10}$ \\
\texttt{u\_rho\_xa} & $u_x^{\rho, a < 15}$ & Latent & $n$ & $\mathbb{R}^{n}$ \\
\texttt{u\_alpha\_x} & $u^\alpha_x$ & Latent & $n$ & $\mathbb{R}^{n}$ \\
\texttt{us\_alpha\_x} & & Latent & $n$ & $\mathbb{R}^{n}$ \\
\texttt{u\_alpha\_xs} & $u_x^{\alpha, s = \text{M}}$ & Latent & $n$ & $\mathbb{R}^{n}$ \\
\texttt{us\_alpha\_xs} & & Latent & $n$ & $\mathbb{R}^{n}$ \\
\texttt{u\_alpha\_a} & $u^\alpha_a$ & Latent & 13 & $\mathbb{R}^{13}$ \\
\texttt{u\_alpha\_as} & $u_a^{\alpha, s = \text{M}}$ & Latent & 10 & $\mathbb{R}^{10}$ \\
\texttt{u\_alpha\_xa} & $u_x^{\alpha, a < 15}$ & Latent & $n$ & $\mathbb{R}^{n}$ \\
\texttt{ui\_lambda\_x} & $u_x^\lambda$ & Latent & $n$ & $\mathbb{R}^{n}$ \\
...
\caption{Correspondence between mathematical notation and variable names used in our \texttt{TMB} code. The total number of hyperparameters is 24, and the total number of latent field parameters is $51 + 14n$.}
\end{table}
```
...
This section contains C++ for the negative log posterior of the simplified Naomi model we describe in Section \ref{sec:math}.
For ease of understanding, Table \ref{tab:tmb-lookup} provides correspondence between the mathematical notation used and the variable names used, for all hyperparameters and latent field parameters.
For further reference on the `TMB` software see `https://kaskr.github.io/adcomp/_book`.

```{cpp, echo=FALSE, eval=FALSE, output.var="ex", code=readLines("naomi_simple.cpp")}
