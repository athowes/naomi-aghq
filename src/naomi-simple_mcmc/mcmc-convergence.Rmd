...
ess_histogram <- data.frame(mcmc_summary) %>%
  ...
  ggplot(aes(x = n_eff)) +
    ...
    scale_x_log10() +
    labs(x = "Effective sample size (log 10 scale)", y = "Count", tag = "B")
...
```{r}
worst_ess_par <- names(which.min(mcmc_summary[, "n_eff"]))
worst_rhat_par <- names(which.max(rhats))

worst_df <- bayesplot::mcmc_trace_data(mcmc, pars = c(worst_ess_par, worst_rhat_par)) |>
  mutate(
    chain_group = case_when(
      chain %in% 1:6 ~ "Chains 1–6",
      chain %in% 7:12 ~ "Chains 7–12",
      chain %in% 13:18 ~ "Chains 13–18"
    ),
    chain_id = as.numeric(chain) %% 6 + 1
  )

worst <- ggplot(worst_df, aes(x = iteration, y = value, col = as.factor(chain_id))) +
  geom_line() +
  scale_color_manual(values = cbpalette) +
  facet_grid(chain_group ~ parameter) +
  guides(col = "none") +
  labs(x = "Iteration", y = "") +
  theme_minimal()

ggsave("worst-trace.png", worst, h = 4.5, w = 6.25)
```
...
pairs_df <- rstan::extract(mcmc, pars = c("log_sigma_rho_a", "logit_phi_rho_a")) %>%
  as.data.frame()
