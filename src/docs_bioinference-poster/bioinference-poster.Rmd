...
```{r aghq, echo=FALSE, fig.cap='Demonstration of PCA-AGHQ.'}
mu <- c(1, 1.5)
...
* Small country but still has latent field $\text{dim}(x) = 491$ and hyperparameters $\text{dim}(\theta) = 24$
* Fit three inference methods (using one C++ template):
  * TMB (`r round(time_taken$TMB, 0)` secs)
  * PCA-AGHQ (`r round(time_taken$aghq, 0)` hour): $k = 3, s = 8$
